...
#        #Perform Version Check first
    ...
    def addFriend(self, friend):
        ...
        self.send(dict(command="social_add", friend=friend)) #LATER: Use this line instead
        self.usersUpdated.emit([friend])

    def addFoe(self, foe):
        '''Adding a new foe by user'''
        self.foes.append(foe)
        self.send(dict(command="social_add", foe=foe)) #LATER: Use this line instead
        self.usersUpdated.emit([foe])

    def remFriend(self, friend):
        '''Removal of a friend by user'''
        self.friends.remove(friend)
        self.send(dict(command="social_remove", friend=friend)) #LATER: Use this line instead
        self.usersUpdated.emit([friend])

    def remFoe(self, foe):
        '''Removal of a foe by user'''
        self.foes.remove(foe)
        self.send(dict(command="social_remove", foe=foe)) #LATER: Use this line instead
        self.usersUpdated.emit([foe])
